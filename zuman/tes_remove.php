<?php

require 'db.php';
$user_id=$_GET['id'];


$select_photo="SELECT photo FROM testimonial WHERE id=$user_id";
$del_photo_query= mysqli_query($db_connection,$select_photo);
$after_assoc=mysqli_fetch_assoc($del_photo_query); 

$delet_from_location="uploads/testimonial/".$after_assoc['photo'];
unlink($delet_from_location);

$remove="DELETE FROM testimonial WHERE id=$user_id";
$query=mysqli_query($db_connection,$remove);
header("location:tes_view.php");





?>